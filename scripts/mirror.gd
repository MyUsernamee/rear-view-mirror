...
#	mirror_camera.fov = player_camera.fov;
	...
	if config_dirty:
		...
		var quad_material:ShaderMaterial = get_surface_override_material(surface_index)
	...
	size.y = mesh.get_aabb().size.y
	size.x = mesh.get_aabb().size.x
	# Transform mirror camera to opposite side of mirror plane

	# Convert camera the local space
	var position = global_transform.orthonormalized().basis.inverse() * (player_camera.global_position - global_position);
	mirror_camera.global_transform = global_transform.orthonormalized() * Transform3D(Basis.FLIP_X, position * Vector3(1, 1, -1))
	mirror_camera.set_frustum(size.y * image_scaling, Vector2(position.x, -position.y), abs(position.z), 100000);
	RenderingServer.camera_set_transform(mirror_camera.get_camera_rid(), mirror_camera.global_transform)
